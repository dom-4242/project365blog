  ...
  return (
    <div
      className={`rounded-2xl border p-4 flex items-start gap-3 ${
        ok ? 'bg-movement-50 border-movement-200' : 'bg-sand-50 border-sand-200'
      }`}
    >
      <span
        className={`mt-0.5 flex-none w-5 h-5 rounded-full flex items-center justify-center text-xs ${
          ok ? 'bg-movement-500 text-white' : 'bg-sand-300 text-white'
        }`}
      >
        {ok ? '✓' : '·'}
      </span>
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-sm font-semibold mt-0.5 ${ok ? 'text-movement-700' : 'text-sand-400'}`}>
      ...
      {!ok && (
        <Link
          href={href}
          className="shrink-0 text-xs px-2.5 py-1 bg-white border border-sand-200 rounded-lg text-sand-600 hover:border-sand-300 hover:text-[#1a1714] transition-colors"
        >
          Erfassen
        </Link>
      )}
...
function StatCard({ value, label, sub }: StatCardProps) {
  return (
    <div className="bg-white rounded-2xl border border-sand-200 p-4 text-center">
      <p className="font-display text-3xl font-bold text-[#1a1714]">{value}</p>
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Link
            href="/admin/entries/new"
            className="group bg-white rounded-2xl border border-sand-200 p-5 flex items-center gap-4 hover:border-nutrition-300 hover:shadow-sm transition-all"
          >
            <div className="flex-none w-10 h-10 bg-nutrition-100 rounded-xl flex items-center justify-center group-hover:bg-nutrition-200 transition-colors">
              <svg className="w-5 h-5 text-nutrition-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
              </svg>
            </div>
            <div>
              <p className="font-display text-sm font-semibold text-[#1a1714]">Neuer Eintrag</p>
              <p className="text-xs text-sand-500 mt-0.5">Journal-Eintrag mit Habits erfassen</p>
            </div>
          </Link>
          <Link
            href={`/admin/metrics?date=${today}`}
            className="group bg-white rounded-2xl border border-sand-200 p-5 flex items-center gap-4 hover:border-movement-300 hover:shadow-sm transition-all"
          >
            <div className="flex-none w-10 h-10 bg-movement-100 rounded-xl flex items-center justify-center group-hover:bg-movement-200 transition-colors">
              <svg className="w-5 h-5 text-movement-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
              </svg>
            </div>
            <div>
              <p className="font-display text-sm font-semibold text-[#1a1714]">Metriken erfassen</p>
      ...
      {recentEntries.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-semibold text-sand-400 uppercase tracking-wide">
              Letzte Einträge
            </h2>
            <Link href="/admin/entries" className="text-xs text-sand-500 hover:text-[#1a1714] transition-colors">
              Alle anzeigen →
            </Link>
          </div>
            ...
            {recentEntries.map((entry) => {
              ...
              return (
                <div key={entry.id} className="px-5 py-3 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#1a1714] truncate">{entry.title}</p>
                    <p className="text-xs text-sand-400">{formatDateShort(entry.date)}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {!entry.published && (
                      <span className="text-xs px-1.5 py-0.5 bg-sand-100 text-sand-500 rounded">
                        Entwurf
                      </span>
                    )}
                    <Link
                      href={`/admin/entries/${entry.id}/edit`}
                      className="text-xs text-sand-400 hover:text-[#1a1714] transition-colors"
                    >
                      Bearbeiten
                    </Link>
                  </div>
      ...
      {latestMetrics && (
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/admin/metrics" className="text-xs text-sand-500 hover:text-[#1a1714] transition-colors">
              Alle anzeigen →
            </Link>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {latestMetrics.weight != null && (
                <div>
                  ...
                  <p className="text-base font-semibold text-[#1a1714] mt-0.5">{latestMetrics.weight} kg</p>
                </div>
              )}
              {latestMetrics.bodyFat != null && (
                <div>
                  <p className="text-xs text-sand-500">Körperfett</p>
                  <p className="text-base font-semibold text-[#1a1714] mt-0.5">{latestMetrics.bodyFat}%</p>
                </div>
              )}
              {latestMetrics.steps != null && (
                <div>
                  <p className="text-xs text-sand-500">Schritte</p>
                  <p className="text-base font-semibold text-[#1a1714] mt-0.5">
              ...
              {latestMetrics.restingHR != null && (
                <div>
                  ...
                  <p className="text-base font-semibold text-[#1a1714] mt-0.5">{latestMetrics.restingHR} bpm</p>
                </div>
              )}
            </div>
          </div>
