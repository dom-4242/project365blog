...
export function HomeTabs({ labels, entriesContent, habitsContent, metricsContent }: HomeTabsProps) {
  ...
  return (
    <div>
      {/* Tab Bar */}
      <div className="flex gap-1 border-b border-sand-200 dark:border-[#313244] mb-8">
        {tabs.map(({ id, label }) => (
          <button
            key={id}
            ...
            className={clsx(
              ...
              activeTab === id
                ...
                : 'border-transparent text-sand-500 hover:text-ctp-text hover:border-sand-300 dark:hover:border-[#585b70]'
            )}
          >
            {label}
          </button>
